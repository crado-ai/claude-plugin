...
- **`publish_page`** — publish a complete, self-contained HTML document and get back `url`, `page_id`, `revision`, `visibility`, `visibility_source` and `sharing` — one sentence saying who can open the page; when it is not public, everyone else who opens the link gets a not-found page, so tell the user and offer to make it public (the sentence names the call). `update_page` and `publish_canvas` return the same fields. Arguments: `title`, `html`, optional `collection`, `workspace`, `source`, `visibility` (`public`, `workspace` or `only_you`; omit it to take the workspace default), `template` (the key of a frame template whose `{{ body }}` wraps your HTML).
